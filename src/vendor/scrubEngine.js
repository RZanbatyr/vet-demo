...
function mountScrollWorld(container, config) {
  ...
  // Release the blob and detach the <video> once a scene is safely off-screen.
  // On iOS Safari every live <video> is a decoder handle and the blob sits in
  // memory — with 4 dive clips + connectors this piles up and starves the
  // decoder that's currently scrubbing. We keep the still poster in place so
  // when the user scrolls back the scene re-loads cleanly via loadClip().
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      const far = y < s.start - 2.6 * vh || y > s.end + 2.6 * vh;
      if (near) loadClip(s);
      else if (mobile && far) unloadClip(s);
